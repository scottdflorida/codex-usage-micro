...
enum MenuBarGaugeRenderer {
    ...
    static let statusItemLength = CGFloat(MenuBarItemLayout.standard.statusItemWidth)
    ...
    static func image(for displayState: MenuBarDisplayState) -> NSImage {
        ...
        let staleBadgeWidth =
            ...
            : 0
        let availableBrandWidth = layout.brandSlotWidth - staleBadgeWidth
        ...
        let image = NSImage(size: imageSize, flipped: false) { bounds in
            ...
            if displayState.freshness == .stale {
                drawStaleBadge(in: bounds, brandSlotWidth: layout.brandSlotWidth)
            }

            let trackRect = NSRect(
                x: bounds.minX + layout.gaugeOriginX,
